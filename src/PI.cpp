...
// r = sqrt(x)
void my_sqrt_ui(mpf_t r, unsigned x)
{
...
void fac_init_size(fac_t f, int s)
{
    if (s<INIT_FACS)
        s=INIT_FACS;
...
// f = base^pow
void fac_set_bp(fac_t f, unsigned base, int pow)
{
    int i;
...
void bs_mul(mpz_t r, int a, int b)
{
    int i, j;
    if (b-a<=32) {
        mpz_set_ui(r, 1);
...
{
    int i, j, k, c;
    fac_resize(fmul, min(fp[0].num_facs, fg[0].num_facs));
    for (i=j=k=0; i<fp[0].num_facs && j<fg[0].num_facs; ) {
        if (fp[0].fac[i] == fg[0].fac[j]) {
            c = min(fp[0].pow[i], fg[0].pow[j]);
            fp[0].pow[i] -= c;
            fg[0].pow[j] -= c;
            fmul[0].fac[k] = fp[0].fac[i];
            fmul[0].pow[k] = c;
            i++; j++; k++;
        } else if (fp[0].fac[i] < fg[0].fac[j]) {
            i++;
        } else {
            j++;
        }
    ...
    fmul[0].num_facs = k;
    assert(k <= fmul[0].max_facs);
    
    if (fmul[0].num_facs) {
        bs_mul(gcd, 0, fmul[0].num_facs);
...
// binary splitting
void bs(unsigned a, unsigned b, unsigned gflag, int level)
{
    unsigned i, mid;
...
void build_sieve(int n, sieve_t *s)
{
    int m, i, j, k;
...
// [[Rcpp::export]]
string pi_approx(int d)
{
    mpf_t  pi, qi;
    int i, depth=1, terms;
    unsigned psize, qsize;
    mp_exp_t t0;
    
    char *pi_str;
    pi_str =(char *) malloc(sizeof(char)*(d+4));
    
    out = 1;
    terms = d/DIGITS_PER_ITER;
    while ((1<<depth)<terms) depth++;
    depth++;
    percent = terms/100.0;
    ...
    mpf_set_default_prec((int)(d*BITS_PER_DIGIT+16));
    mpf_init(pi);
    mpf_init(qi);
    ...
    mpf_set_z(pi, p1);
    mpf_set_z(qi, q1);
    mpf_div_2exp(pi, pi, mpz_sizeinbase(q1,2));
    mpf_div_2exp(qi, qi, mpz_sizeinbase(q1,2));
    
    /* free stacks top and stacks */
    mpz_clear(pstack[0]);
    mpz_clear(qstack[0]);
    mpz_clear(gstack[0]);
    fac_clear(fpstack[0]);
    ...
    mpf_get_str(pi_str,&t0,10,d+4,qi);
    pi_str[d+3]='\0'; pi_str[0]='.';
